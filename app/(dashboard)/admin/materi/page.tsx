  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Kelola Materi Belajar</h1>
          <p className="text-gray-500">Unggah modul, PDF, atau materi pelajaran lainnya untuk siswa</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        {/* Form Upload Materi */}
        <section>
          <h2 className="text-xl font-bold text-gray-800 mb-4 px-2">Unggah Materi Baru</h2>
        ...
        <section className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mt-8">
          <div className="p-6 border-b border-gray-100">
            <h3 className="font-bold text-gray-800 text-lg">Materi Terbaru</h3>
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-gray-500 uppercase bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-3">Diunggah Pada</th>
                </tr>
              </thead>
              <tbody>
                {existingMateri.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-gray-400">Belum ada materi yang diunggah.</td>
                  </tr>
                ) : (
                  existingMateri.map((m: any) => (
                    <tr key={m._id.toString()} className="border-b hover:bg-gray-50">
                      <td className="px-6 py-4 font-medium text-gray-900">{m.judul}</td>
                      ...
                      <td className="px-6 py-4">
                        <a href={m.file_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
