...
import { syncStudentsFromApiAction } from '@/actions/admin-actions';
import { useState } from 'react';

export default function SyncSiswaButton() {
  const [isSyncing, setIsSyncing] = useState(false);

  const handleSync = async () => {
    if (!confirm('Apakah Anda yakin ingin menarik data siswa dari API pusat?')) return;
    
    setIsSyncing(true);
    try {
      const res = await syncStudentsFromApiAction();
      alert(res.message);
    } catch (error) {
      alert('Terjadi kesalahan saat sinkronisasi data siswa.');
    } finally {
      setIsSyncing(false);
    }
  };
