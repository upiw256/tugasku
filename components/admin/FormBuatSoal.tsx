  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 pb-20">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-2xl px-4">
        <button type="submit" disabled={isLoading} className={`w-full py-4 rounded-xl text-white font-bold text-lg shadow-xl transition-all ${isLoading ? 'bg-gray-400' : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:scale-[1.02] active:scale-[0.98]'}`}>
          {isLoading ? 'Sedang Memproses...' : '🚀 Simpan dan Publikasi Kuis'}
        </button>
      </div>
    </form>
  );
